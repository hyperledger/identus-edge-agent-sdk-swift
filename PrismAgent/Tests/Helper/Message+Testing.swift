...
extension Domain.Message: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.init(
            id: id,
            piuri: piuri,
            from: from.map { didFromString(str: $0) },
            to: to.map { didFromString(str: $0) },
            fromPrior: fromPrior,
            body: body,
...
private func didFromString(str: String) -> DID {
    let components = str.components(separatedBy: ":")
    return DID(
        schema: components.first!,
        method: components[1],
        methodId: components.last!
    )
}
